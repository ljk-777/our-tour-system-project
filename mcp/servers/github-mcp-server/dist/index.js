...
import { z } from "zod";
import { GitHubClient, formatPR, formatIssue, truncate, decodeBase64, } from "./github-client.js";
// ─── Bootstrap ────────────────────────────────────────────────────────────────
...
server.registerTool("github_list_prs", {
    description: "List pull requests for a GitHub repository. Returns title, number, state, author, " +
...
server.registerTool("github_get_pr", {
    description: "Get full details of a single pull request including description, diff stats, labels, " +
...
server.registerTool("github_get_pr_diff", {
    description: "Get the list of changed files for a pull request, including diff patches. " +
...
server.registerTool("github_get_pr_comments", {
    description: "Get inline review comments left on a pull request (line-level code comments). " +
...
server.registerTool("github_list_issues", {
    description: "List issues for a GitHub repository. Note: GitHub's API also returns pull requests " +
...
server.registerTool("github_get_issue_comments", {
    description: "Get comments on a GitHub issue or pull request (general discussion comments, " +
...
server.registerTool("github_get_repo", {
    description: "Get metadata for a GitHub repository: description, stars, forks, open issues, " +
...
server.registerTool("github_list_repos", {
    ...
    const lines = repos.map((r) => `- **${r.full_name}** ⭐${r.stargazers_count} | ${r.language ?? "?"} | ${r.description?.slice(0, 80) ?? ""}`);
    const text = [
...
server.registerTool("github_list_commits", {
    description: "List commits for a repository, optionally filtered by branch/SHA or file path.",
    inputSchema: {
        ...OwnerRepo,
        sha: z
            .string()
...
server.registerTool("github_get_file", {
    description: "Get the text content of a file from a GitHub repository. " +
...
server.registerTool("github_search_code", {
    description: "Search for code across GitHub repositories. Use GitHub search qualifiers in the query " +
        "(e.g. 'dijkstra repo:ljk-777/our-tour-system-project', 'useState language:typescript').",
    inputSchema: {
        query: z
            .string()
            .min(1)
            .describe("GitHub code search query. Supports qualifiers: repo:owner/name, language:ts, path:src/, filename:index.ts"),
        ...Pagination,
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
