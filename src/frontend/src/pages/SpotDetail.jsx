            ...
            <div>
              ...
              <div className="mb-3 flex items-center gap-2">
                <input
                  ...
                  placeholder="按类别过滤，如：餐厅、超市、厕所、博物馆…"
                  className="flex-1 text-sm px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:border-blue-400"
                />
                {nearbyKeyword && (
                  <button onClick={() => setNearbyKeyword('')} className="text-xs text-gray-400 hover:text-gray-600 px-2">清除</button>
                )}
              </div>
              {nearby.length === 0 ? (
                ...
                const filtered = nearby.filter(item =>
                  !nearbyKeyword.trim() ||
                  item.name?.includes(nearbyKeyword) ||
                  (TYPE_LABELS[item.type] || item.type || '').includes(nearbyKeyword) ||
                  item.type?.includes(nearbyKeyword)
                );
                return filtered.length === 0 ? (
                  <div className="text-center py-10 text-gray-400">
                    <div className="text-3xl mb-2">🔍</div>
                    <p>没有找到匹配「{nearbyKeyword}」的场所</p>
                  </div>
                ) : (
